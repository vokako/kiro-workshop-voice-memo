...
> - 可以创建多条备忘录（自动以时间戳命名）
> - 备忘录列表展示，可切换查看
> - 每条备忘录支持 Markdown 文本编辑
> - 支持语音录入，点击"停止"后自动发送后端转写，结果追加到备忘录
> - 支持上传音频文件转写
> - 录音时显示实时波形
> - 数据持久化到本地文件，重新打开可加载
> - 不需要写测试，代码越简单越好
...
> - 模型：sherpa-onnx SenseVoice int8，路径 `models/sherpa-onnx-sense-voice-zh-en-ja-ko-yue-int8-2024-07-17/`
> - 模型下载：`https://du7u4d2q1sjz6.cloudfront.net/sherpa-onnx-sense-voice-zh-en-ja-ko-yue-int8-2024-07-17.tar.bz2`
> - 请先检查并准备好运行环境：确认 Python 3、ffmpeg 已安装，安装 pip 依赖（sherpa-onnx fastapi uvicorn），下载模型解压到 `models/` 目录
>
> **存储规则：**
...
3. 硬性约束（踩坑经验）对 Agent 的实现质量有什么影响？
