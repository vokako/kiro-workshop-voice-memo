...
**开发分两个阶段喂提示词**：先做纯文字的备忘录（跑通验证），再加语音录入。这样出问题时容易定位是不是语音部分。
...
> **环境配置交给 Kiro**：Python 依赖、ffmpeg、模型下载等准备工作不需要手动操作，在第 2 步输入需求时一并告诉 Kiro，让它来处理。
...
本工作坊把需求**分两次**喂给 Kiro：先用【阶段一】提示词做出纯文字备忘录并跑通，再用【阶段二】提示词加语音。

先把【阶段一】提示词粘贴到 Spec 会话中：
...
> - 请先检查并准备运行环境：确认 Python 3 已安装，安装 pip 依赖（fastapi uvicorn）
>
> **存储规则：**
> - 所有数据存在 `data/` 目录，每条备忘录一个子目录（如 `data/2024-07-18_143052/`）
> - 文字笔记存为 `note.md`，用 JSON 索引文件管理元数据

---

按下面 2.3 ~ 2.6 生成并执行任务，再用第 3 步启动测试。**阶段一跑通后**，把【阶段二】提示词粘贴进去，同样走一遍 Quick Plan → Run All Tasks → 启动测试：

---

> **【阶段二：加语音】**（阶段一跑通后再粘贴）
>
> 在上面备忘录应用基础上新增语音录入与转写，不要改动已 work 的功能。
>
> **功能需求：**
...
>
> 如果想跳过所有权限确认，可以在 Kiro 设置中将工具设为自动信任（Trust All Tools），这样所有工具调用会自动执行：
>
...
> **注意**：首次运行时可能还需要下载模型等准备操作，具体看 Kiro 执行任务时的输出提示进行调整。
