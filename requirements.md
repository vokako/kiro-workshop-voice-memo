...
分两阶段开发,先把稳的做出来,再加有风险的语音:

- **阶段一:备忘录应用**(纯笔记,无语音 / AI 依赖)——先完整跑通并验证。
- **阶段二:加入语音录入与转写**(sherpa-onnx 本地识别 + ffmpeg)——在阶段一基础上只做增量。

衔接:阶段一是"打字的备忘录",阶段二让它能"说话录入"。
**先把阶段一完整跑通、验证 OK,再进阶段二**;这样一旦出问题,能确定就是语音部分。

## 公共技术架构

- 后端:Python + FastAPI;前端纯 HTML/CSS/JS 单页面,由 FastAPI 直接托管,不需要 Node.js 或任何前端构建工具。
...
一个本地运行的会议备忘录工具。用户可以创建多条备忘录,每条支持 Markdown 文本编辑。所有数据完全本地,不依赖云服务。
...
- 重新打开应用时能加载之前保存的所有备忘录;编辑或新增内容后自动保存

## 运行前提

- Python 3 + `pip install fastapi uvicorn`(阶段一**不需要** sherpa-onnx / ffmpeg)

## 阶段一完成标准(达成后再进阶段二)

能创建备忘录、Markdown 编辑并预览、列表切换、刷新 / 重启后数据仍在。

---

# 阶段二:加入语音录入与转写

> 前提:阶段一已完整跑通。本阶段**只新增**语音相关部分,不改动阶段一已 work 的功能。
...
- 语音识别:sherpa-onnx SenseVoice int8,固定路径 `models/sherpa-onnx-sense-voice-zh-en-ja-ko-yue-int8-2024-07-17/`
- 模型下载:`https://du7u4d2q1sjz6.cloudfront.net/sherpa-onnx-sense-voice-zh-en-ja-ko-yue-int8-2024-07-17.tar.bz2`,解压后放到 `models/` 目录
- 新增依赖:`pip install sherpa-onnx`、`ffmpeg`
...
选择麦克风后录音波形有波动,点击停止后自动转写并把文字追加到当前备忘录;上传音频也能转写。
